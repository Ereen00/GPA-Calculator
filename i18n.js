...
(function (global) {
  ...
  var DICT = {
    ...
    'editor.lessonPlaceholder': { tr: 'Ders kodu veya adı (örn. MATH 101, Calculus)', en: 'Course code or name (e.g. MATH 101, Calculus)' },
    'suggest.credit': { tr: '{c} kr', en: '{c} cr' },
    'suggest.fromPlan': { tr: 'Planınızdan', en: 'From your plan' },
    ...
    /* ---- İstatistik: ders alanlarına göre performans ---- */
    'stats.subject.title': { tr: 'Ders Alanlarına Göre Performans', en: 'Performance by Subject Area' },
    'stats.subject.badge': { tr: 'Alan Karşılaştırması', en: 'Subject Comparison' },
    'stats.subject.lead': { tr: 'Ders kodunun harf kısmı (MATH, CHEM, CMPE…) bir alan sayılır: MATH101, MATH201 ve MATH301 aynı alanda, kredi ağırlıklı tek bir ortalamada birleşir. Tekrar ettiğiniz derslerde yalnızca ortalamaya sayılan son not kullanılır. Dikey çizgi genel ortalamanızı (GNO) gösterir.', en: 'The letter part of a course code (MATH, CHEM, CMPE…) is treated as a subject area: MATH101, MATH201 and MATH301 are merged into one credit-weighted average. For retaken courses only the grade that counts toward your GPA is used. The vertical line marks your cumulative GPA.' },
    'stats.subject.ds': { tr: 'Alan ortalaması', en: 'Subject average' },
    'stats.subject.gpaLine': { tr: 'GNO {g}', en: 'GPA {g}' },
    'stats.subject.tooltip': { tr: 'Ortalama {g} · {n} ders · {c} kredi', en: 'Average {g} · {n} courses · {c} credits' },
    'stats.subject.none': { tr: 'Alan analizi için harfle başlayan ders kodlarına (ör. MATH101) ve notu girilmiş derslere ihtiyaç var.', en: 'Subject analysis needs graded courses whose codes start with letters (e.g. MATH101).' },
    'stats.subject.count': { tr: 'Derslerinizi <strong>{n} alan</strong> altında topladık.', en: 'Your courses fall into <strong>{n} subject areas</strong>.' },
    'stats.subject.truncated': { tr: 'Grafikte en çok krediye sahip {n} alan gösteriliyor; hepsi aşağıdaki karşılaştırmada seçilebilir.', en: 'The chart shows the {n} areas with the most credits; all of them can be picked in the comparison below.' },
    'stats.subject.best': { tr: 'En güçlü alanınız <strong>{s}</strong> ({g}).', en: 'Your strongest area is <strong>{s}</strong> ({g}).' },
    'stats.subject.worst': { tr: 'En çok zorlandığınız alan <strong>{s}</strong> ({g}).', en: 'The area you find hardest is <strong>{s}</strong> ({g}).' },
    'stats.subject.without': { tr: '{s} dersleri hariç tutulsaydı GNO\'nuz <strong>{g}</strong> olurdu ({d}).', en: 'Without your {s} courses your GPA would be <strong>{g}</strong> ({d}).' },
    'stats.subject.compareTitle': { tr: 'İki alanı yan yana karşılaştırın', en: 'Compare two areas side by side' },
    'stats.subject.pickA': { tr: 'Birinci alan', en: 'First area' },
    'stats.subject.pickB': { tr: 'İkinci alan', en: 'Second area' },
    'stats.subject.optionMeta': { tr: '{c} kr', en: '{c} cr' },
    'stats.subject.vsGpa': { tr: 'GNO\'ya göre {d}', en: '{d} vs. GPA' },
    'stats.subject.courses': { tr: 'Ders', en: 'Courses' },
    'stats.subject.credits': { tr: 'Kredi', en: 'Credits' },
    'stats.subject.top': { tr: 'En iyi', en: 'Best' },
    'stats.subject.low': { tr: 'En düşük', en: 'Lowest' },
    'stats.subject.same': { tr: 'Karşılaştırmak için iki farklı alan seçin.', en: 'Pick two different areas to compare.' },
    'stats.subject.equal': { tr: '<strong>{a}</strong> ve <strong>{b}</strong> ortalamalarınız eşit.', en: 'Your <strong>{a}</strong> and <strong>{b}</strong> averages are equal.' },
    'stats.subject.diff': { tr: '<strong>{hi}</strong> ortalamanız <strong>{lo}</strong> ortalamanızdan <strong>{d} puan</strong> yüksek.', en: 'Your <strong>{hi}</strong> average is <strong>{d} points</strong> higher than your <strong>{lo}</strong> average.' },
    ...
    'stats.card.nextSpaNoTrend': { tr: 'Son Dönem Ortalaması (trend için tek dönem yeterli değil)', en: 'Last term average (one term is not enough for a trend)' },
    ...
    'stats.trend.insufficient': { tr: 'Trend hesaplamak için en az iki dönem gerekir.', en: 'At least two terms are needed to compute a trend.' },
